...
public class EspectadorService {
    ...
    public ArrayList<Espectador> crearEspectadores() {
        int n = 50; //Cantidad de espectadores que se desean generar
        String[] names = new String[n];
        int[] age = new int[n];
        double[] money = new double[n];
        Random random = new Random();
        ArrayList<Espectador> spectatorList = new ArrayList();

        for (int i = 0; i < names.length; i++) {
            Espectador spectator = new Espectador();
            spectator.setName(crearNombresRandom(random));
            spectator.setAge(random.nextInt(71) + 6);
            spectator.setMoney(random.nextFloat() * 100);           
    ...
    public String crearNombresRandom(Random r) {
        String[] nombres = {"John", "Emma", "Michael", "Sophia", "David", "Olivia", "Daniel", "Ava", "Matthew", "Isabella"};
        String[] apellidos = {"Smith", "Johnson", "Brown", "Taylor", "Miller", "Davis", "Garcia", "Martinez", "Lee", "Wilson"};
    ...
    public void printArray(ArrayList<Espectador> a){
